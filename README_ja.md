...
nyancobot は複数のメッセージングプラットフォームに標準対応しています。必要なものだけインストールできます：

```bash
# 特定のプラットフォームのみインストール
pip install nyancobot[slack]     # Slack のみ
pip install nyancobot[discord]   # Discord のみ
pip install nyancobot[line]      # LINE のみ
pip install nyancobot[whatsapp]  # WhatsApp のみ
...
**Slack**: [Slack API](https://api.slack.com/apps) でボットトークンを取得 → Socket Mode を有効化 → `xoxb-` と `xapp-` トークンをコピー。

**Discord**: [Discord Developer Portal](https://discord.com/developers/applications) でボットを作成 → トークンをコピー → Message Content Intent を有効化。

**LINE**: [LINE Developers](https://developers.line.biz/) でチャンネルを作成 → Channel Access Token と Secret を取得 → Webhook URL を設定。

**WhatsApp**: [Meta for Developers](https://developers.facebook.com/) で登録 → トークンと Phone Number ID を取得 → Webhook を設定。

### 設定方法

`~/.nyancobot/config/config.json` を更新：
